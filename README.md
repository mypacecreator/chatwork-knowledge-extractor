...
| `FILTER_BOILERPLATE_THRESHOLD` | - | 定型文で**始まる**メッセージを救済する閾値。この文字数以上なら定型文パターンにマッチしても通す。デフォルト`50` |
...
- **短すぎるメッセージ**（デフォルト: 10文字未満）
  - 例: `!`, `w`, `OK`, `了解です`, `承知しました`
- **定型文・相槌**（正規表現マッチング、40+パターン）
  - 一般的な定型文: `お疲れ様です`, `よろしくお願いします`, `ありがとうございました`
  - 作業完了報告: `完了です`, `対応しました`, `確認しました`, `更新しました`
  - Web制作特有: `アップしました`, `デプロイしました`, `コミットしました`, `反映しました`
  - 短い返信: `はい`, `そうですね`, `大丈夫です`, `見ました`, `確認しておきます`
- **記号・絵文字のみ**
  - 例: `!!!`, `👍`, `🙏`, `ww`

### 切り詰め処理

- **長すぎるメッセージ**（デフォルト: 300文字超）は、300文字で切り詰め
